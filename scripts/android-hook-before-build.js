	...
	fs.readFile(target, 'utf-8', function(error, data){
		...
		if (modifiedData.indexOf("onLowMemory") == -1) {
			...
			const onLowMemoryBlock = [
				...
				i+i+'this.appView.loadUrl("javascript:cordova.fireDocumentEvent(\'memorywarning\', { mtype: \'lowMemory\' });");',
				i+'}',
				''
			].join("\n")
		...
		if (modifiedData.indexOf("onTrimMemory") == -1) {
			...
			const onTrimMemoryBlock = [
				...
				i+i+i+i+'this.appView.loadUrl("javascript:cordova.fireDocumentEvent(\'memorywarning\', { mtype: \'trimMemory\', level: " + level + " });");',
				i+i+i+i+'break;',
				i+i+'}',
				i+'}',
				''
